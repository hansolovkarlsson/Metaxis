...
`scratch/daily-standup.md` is written at the end of the previous working day to
...
template with `{hole}` splices on the output side. So a directive can never be
read as the thing it is declaring.
...
it, then the eleven scripts in `tests/`: `errors.sh`, `hygiene.sh`, `docs.sh`,
`pascal.sh`, `asm.sh`, `python.sh`, `basic.sh`, `island.sh`, `cpp.sh`,
`mini.sh` and `scale.sh`. Eight
...
print and compares every labelled block to the file it quotes, and `scale.sh` expands one input large enough for a quadratic to show.
