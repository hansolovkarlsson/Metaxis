...
The invented or borrowed-from-elsewhere names and the acronyms cover the whole
survey between them. The names: **Stratego**, **Spoofax**, **Rascal**, **Ohm**,
**Comby**, **Katahdin**, **Coccinelle**, **Silver**, **Seed7**, **Nanopass**.
The acronyms: **TXL**, **ANTLR**, **SDF**, **SmPL**, **DMS**, **META II**. The
...
nothing else: there is no category listing to browse. So the name has to be a
term that returns the tool when typed into a search box. Coccinelle means
...
- **Landin**: Peter Landin, and three connections where Schorre has one.
  *"The Next 700 Programming Languages"* (1966) is a language **schema**
  ...
  functional programming, which this tool is not: ISWIM, SECD, the ML lineage.
...
`pro`/`syn`/`lingua` morphemes, then `genesis`. Every meaning-bearing word in or
near this domain came back occupied. That is the same statement as the one
...
view (set of templates) — all without recompiling the translator."* <!-- as written -->
...
already unusually serious about this: most of the scripts in `tests/` run
what they produced rather than diffing it. A round-trip check is the same instinct
...
| — | Bidirectional rules (§3.6) | — | declined: wants the output language's grammar | <!-- as written -->
