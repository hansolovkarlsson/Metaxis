...
short account of the concepts in the order they depend on each other, because
the words only make sense against that picture: what a grammar is, how a file
is turned into tokens, how tokens are turned into structure, and what this
tool does with that structure. The second half is the words,
...
category that the rules define in terms of other categories and terminals:
*expression*, *statement*. A grammar's job is to say how nonterminals
...
run of digits a *number*. Others are individual **words** like `+` or
...
- **CFG** means *context-free grammar*: the classic kind, where a rule says
  ...
  grammar can be **ambiguous**, allowing two different trees for one text, and
...
`if a then if b then x else y`: which `if` owns the `else`? Every
...
of this where the template *introduces* a name: `{~t}` and `fresh("t")` give a
**fresh name**, one that appears nowhere else. It leaves open the
...
a recursion depth cap and no budget; `mx -t` counts what a parse tried, which
is the measurement a budget would be picked from, and the item that wanted one
settled against itself once that count existed. REFERENCE §7,
