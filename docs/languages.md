...
| **C's preprocessor** | whole, as its own language | Stage 6. `lib/cpp.mx` reads object-like and function-like macros, `#undef`, `#ifdef` and its four companions nested, `#include "file"`, and `#` and `##`, over C it does not otherwise read. It is fit for the reason C itself is not: **a directive is a line with a word at the front**, so property 3's list is `sep`-separated parameters and property 6's classes keep a macro name out of a string, and the one thing it needs that no grammar gives is a table, which the file keeps itself in the store (REFERENCE §8.5). What a rule cannot do is the arm of a conditional it must not read, which is a `raw` hole, and the rescan, which is `expand`. The one thing it **refuses** rather than passes through is a `#elif` whose condition it cannot read, `defined(NAME)` being the only one it can, because that stands inside a conditional it does read and an arm holding it would be taken or dropped whole; everything else it does not read is still C for the compiler behind it, which is the property the whole verdict rests on. `tests/cpp.sh` holds the output to the C compiler's own preprocessor. Its rules live in `lib/`, and no body, so `mx -u lib/cpp.mx -i prog.c` (REFERENCE §9.1) points them at a `.c` that says nothing about any grammar, with no header concatenated in front of it and the messages naming that file's own lines. |
...
| language | why |
| --- | --- |
| **C** | Property 1 at its sharpest: `x * y` is a product or a declaration depending on whether `x` is a typedef, and no grammar decides it. Declarators, `int (*f)(int)`, are readable and unpleasant. **What is fit**: an expression-and-statement subset over types the header names, which is what `examples/clike.mx` and stage 2 read, text-mode rewrites over a real file, and the preprocessor whole, which are three rows in the table above. The preprocessor was the wall this row used to name, a second language interleaved by line whose `#define` changes what the lines after it mean; it came down on 2026-09-07, and what remains here is C proper. [direction.md](direction.md) says why a front end is declined. |
